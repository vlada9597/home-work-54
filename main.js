...
function isDebugMode() {
  ...
  console.log('isDebugMode:', result);
  return result;
}
...
function encodeToBase64(...args) {
   try {
    ...
    console.log('Encoding to Base64:', combined);
...
function encodeToHex(...args) {
    try {
    const combined = args.join(':');
    console.log('Encoding to Hex:', combined);
    const buffer = Buffer.from(combined, 'utf8');
...
function safeDecodeFromBase64(base64String) {
  ...
  try {
    ...
    console.error('safeDecodeFromBase64 error:', error.message);
  }
}
...
function safeDecodeFromHex(hexString) {
  ...
  try {
    ...
    console.error('safeDecodeFromHex error:', error.message);
  }
}
